...
class ReceivableFormDialog extends ConsumerStatefulWidget {
  const ReceivableFormDialog({super.key});

  @override
  ConsumerState<ReceivableFormDialog> createState() => _ReceivableFormDialogState();
}

class _ReceivableFormDialogState extends ConsumerState<ReceivableFormDialog> {
      ...
      content: SingleChildScrollView(
        child: Form(
          ...
          child: Column(
            ...
            children: [
              TextFormField(controller: _nameCtrl, decoration: const InputDecoration(labelText: 'Nombre del Cliente', border: OutlineInputBorder()), validator: (v) => v == null || v.isEmpty ? 'Requerido' : null),
              const SizedBox(height: 8),
              TextFormField(controller: _idCtrl, decoration: const InputDecoration(labelText: 'Cédula / RUC', border: OutlineInputBorder())),
              const SizedBox(height: 8),
              ...
              Row(
                children: [
                  Expanded(
                    child: Text('Emisión: ${_issueDate.day}/${_issueDate.month}/${_issueDate.year}', style: const TextStyle(fontSize: 12)),
                  ),
              ...
              Row(
                children: [
                  Expanded(
                    child: Text('Vence: ${_dueDate.day}/${_dueDate.month}/${_dueDate.year}', style: const TextStyle(fontSize: 12)),
                  ),
                  TextButton(onPressed: () async {
  ...
  Future<void> _submit() async {
    ...
    final receivable = ReceivableModel(
      ...
      clientIdentification: _idCtrl.text.trim().isEmpty ? null : _idCtrl.text.trim(),
    ...
    try {
      await ref.read(receivableServiceProvider).createReceivable(receivable);
